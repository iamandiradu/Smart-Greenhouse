...
$doc.ready(function () { searchHighlight(); });
    ...
    function toolTipWin(url) {
        qtip.pop(url, null, false, true, 7000,
            '<img src="{0}" class="block xMarginAuto toolTipWin" style="max-width: 800px; max-height: 600px;" />' +
    ...
    JQCallWebService('ws/' + ws, ajaxData, function (r) {
        ...
        for (var s = 0; s < sResults.length; s++) {
            ...
            lis += String.format('<li><div class="{3}"><a href="{0}Findpost/{1}">{2}</a></div></li>',
                ...
                (Modernizr.borderimage ? 'overflowHidden nowrap ellipsis' : '')); //Modernizr.borderimage as a test for IE all versions
        }
    ...
    $body
        ...
        .on('mouseover', 'a.authorlink', function () {
            ...
            var linkContent = String.format(
                ...
                (cMemberInfo.memID != memID && cMemberInfo.memID != -1 ?
...
function registerAdminMenus() {
    ...
    var mURLs = ['Default.aspx?tabval=site', 'news.aspx?tabval=editnews', 'forummanager.aspx?tabval=forum', 'user.aspx?tabval=user', 'forumfilter.aspx?tabval=filters',
        'gallery.aspx?tabval=gallery', 'maintenance.aspx?tabval=maintain', 'allsrvmsg.aspx?tabval=allsrvmsg',
        'activation.aspx?tabval=activate'];

    var subTitleUrls = {
        admin0: { titles: ["Site Parameters", "Email Settings", "Theme & Display Options", "Home, Forums, Menu & Breadcrumb", "Posts, List View & Search", "Default Posting Options", 
        ...
        admin4: { titles: ["Abuse & Spam Filters", "Report Tickets", "Post Flagging & Auto Ban", "Captcha & ReCaptcha", "Link & PM Spam Prevention"],
            urls: ["forumfilter.aspx?tabval=filters", "modticket.aspx?tabval=moderator", "warn.aspx?tabval=warn", "captcha.aspx?tabval=captcha", "linkpmspam.aspx?tabval=lpspam"]
        },
